...
import { Dumbbell, Timer, Speaker, Home, Check, Mic, Square, RotateCcw, Zap, ChevronRight, CheckCircle2, Volume2 } from 'lucide-react';
import { Button } from '../components/Button';
import { AudioPlayer } from '../components/AudioPlayer';
import { VisemeGuide } from '../components/VisemeGuide';
import { SessionState } from '../types';
import { WARMUP_EXERCISES, SHADOWING_DRILLS } from '../constants';
...
export const MuscleView: React.FC<Props> = ({ t, sessionState, setSessionState, audioRecorder, setShowReward, onPlayTTS, isPlayingTTS, lang }) => {
  ...
  const warmupInterval = useRef<number>();
  ...
  if (sessionState === SessionState.MUSCLE_MENU) {
    return (
      <div className="flex flex-col items-center justify-center max-w-3xl mx-auto animate-float-up pt-10 w-full">
        <div className="mb-8 flex items-center gap-3"><div className="p-3 bg-amber-500/10 rounded-xl"><Dumbbell className="w-8 h-8 text-amber-500" /></div><h2 className="text-3xl font-bold">{t.muscleMenuTitle}</h2></div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
          <button onClick={handleStartWarmup} className="flex flex-col p-8 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-amber-500/50 hover:shadow-lg transition-all group text-left rtl:text-right"><div className="p-3 bg-amber-500/10 w-fit rounded-xl mb-4 text-amber-500"><Timer className="w-8 h-8" /></div><h3 className="text-xl font-bold mb-2">{t.warmupBtn}</h3><p className="text-sm text-slate-500">{t.warmupDesc}</p></button>
          <button onClick={() => { setCurrentShadowingIndex(0); setSessionState(SessionState.SHADOWING_INTRO); }} className="flex flex-col p-8 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-amber-500/50 hover:shadow-lg transition-all group text-left rtl:text-right"><div className="p-3 bg-amber-500/10 w-fit rounded-xl mb-4 text-amber-500"><Speaker className="w-8 h-8" /></div><h3 className="text-xl font-bold mb-2">{t.shadowingBtn}</h3><p className="text-sm text-slate-500">{t.shadowingDesc}</p></button>
        </div>
  ...
  if (sessionState === SessionState.WARMUP_ACTIVE) {
    ...
    return (
      <div className="max-w-xl mx-auto w-full animate-float-up pt-10 text-center">
        <h2 className="text-3xl font-bold mb-10">{lang === 'ar' ? ex.titleAr : ex.title}</h2>
        <div className="bg-white/80 dark:bg-slate-900/80 rounded-full w-64 h-64 mx-auto flex items-center justify-center border-4 border-amber-500/20 relative mb-10 shadow-2xl">
           <span className="text-6xl font-bold font-mono">{warmupTimer}</span>
        </div>
        <div className="bg-amber-500/10 rounded-2xl p-6 mb-8"><p className="text-lg font-medium">{lang === 'ar' ? ex.instructionAr : ex.instruction}</p></div>
  ...
  if (sessionState === SessionState.WARMUP_COMPLETED) {
    return (
       <div className="flex flex-col items-center justify-center min-h-[50vh] animate-float-up text-center pt-10">
         ...
         <h2 className="text-3xl font-bold mb-4">Warm-up Complete!</h2>
